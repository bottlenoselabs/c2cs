...
public sealed class CSharpCodeGeneratorContext
{
        ...
        where T : CSharpNode
    {
        ...
        {
            throw new UseCaseException($"A handler '{nameof(GenerateCodeHandler)}' does not exist for the type '{type.FullName ?? type.Name}'.");
        }

        var syntax = handler.GenerateCode(this, node);
        if (syntax is not MemberDeclarationSyntax memberSyntax)
        {
            throw new UseCaseException($"The handler '{nameof(GenerateCodeHandler)}' did not return a '{nameof(MemberDeclarationSyntax)}' for the type '{type.FullName ?? type.Name}'.");
        }
