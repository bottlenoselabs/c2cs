...
{
    public class CSharpMapperManaged : ICSharpMapper
	{
        ...
        private static Dictionary<string, string> SystemTypeNameAliases(int bitness)
        {
            ...
            if (Runtime.OperatingSystem == RuntimeOperatingSystem.Windows)
            {
                ...
                aliases.Add("HANDLE", "IntPtr");
				aliases.Add("HWND", "IntPtr");
				aliases.Add("HINSTANCE", "IntPtr");
				aliases.Add("LONG_PTR", "IntPtr");
				aliases.Add("ULONG_PTR", "UIntPtr");
				aliases.Add("SOCKET", "void*");
				aliases.Add("PVOID", "void*");
			}
...
".Trim();
        ...
        private string TypeNameMapPointer(CType type, int sizeOf, bool isSystem)
        {
			...
			if (pointerTypeName.StartsWith("const char*", StringComparison.InvariantCulture))
			{
				return pointerTypeName.Replace("const char*", "[In, MarshalAs(UnmanagedType.LPUTF8Str)]string");
			}

			if (pointerTypeName.StartsWith("const wchar_t*", StringComparison.InvariantCulture))
			{
				return pointerTypeName.Replace("const wchar_t*", "[In, MarshalAs(UnmanagedType.LPWStr)]string");
			}
			...
			if (type.ArraySize > 0)
			{
				if (mappedElementTypeName.StartsWith("const "))
					pointerTypeName = $"[In, MarshalAs(UnmanagedType.LPArray, SizeConst = {type.ArraySize})]" + mappedElementTypeName.Replace("const ", "") + "[]";
				else
					pointerTypeName = $"[MarshalAs(UnmanagedType.LPArray, SizeConst = {type.ArraySize})]" + mappedElementTypeName + "[]";
			}
			else
				pointerTypeName = mappedElementTypeName.Replace("const ","") + pointersTypeName;


            return pointerTypeName;
        }

		public static bool IsCSBuiltinType(string typeName)
		{
			switch (typeName)
			{
				case "char":
